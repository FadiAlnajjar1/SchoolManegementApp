...
public async Task<IActionResult> GetSchools()
{
    var schools = await db.Schools
        .Select(s => new
        {
            ...
            // ✅ Add grades with their sections
            Grades = db.Grades
                .Where(g => g.SchoolId == s.Id)
                .Select(g => new
                {
                    g.Id,
                    g.Name,
                    ...
                    Sections = db.Sections
                        .Where(sec => sec.GradeId == g.Id)
                        .Select(sec => new
                        {
                            sec.Id,
                            sec.Name,
                            sec.LocalSectionNumber
                        })
                        .ToList()
                })
                .ToList()
...
public async Task<IActionResult> GetEmployeeByLocalNumber(int schoolId, int localNumber)
{
    ...
    return Ok(new
    {
        ...
        employeePhone = employeeSchool.Employee?.Phone,
        employeeAddress = employeeSchool.Employee?.Address,
        employeeBirthDate = employeeSchool.Employee?.BirthDate,
...
public async Task<IActionResult> GetSchoolStudents(int schoolId)
{
    // 1. التحقق من وجود المدرسة
    var school = await db.Schools.FindAsync(schoolId);
    if (school is null)
    ...
    var students = await db.Students
        ...
        .Select(s => new
        {
            ...
            LocalGradeNumber = s.Section != null && s.Section.Grade != null ? s.Section.Grade.LocalGradeNumber : (int?)null,
...
[HttpGet("schools/{schoolId:int}/students/{localNumber:int}")]
public async Task<IActionResult> GetStudentByLocalNumber(int schoolId, int localNumber)
{
    // 1. البحث عن الطالب في المدرسة
    var student = await db.Students
        .Include(s => s.School)
        .Include(s => s.Section)
            .ThenInclude(sec => sec!.Grade)
        .FirstOrDefaultAsync(s => s.SchoolId == schoolId &&
                                  s.LocalStudentNumber == localNumber);

    if (student is null)
        return NotFound(new { message = $"لا يوجد طالب برقم {localNumber} في هذه المدرسة" });

    // 2. إرجاع بيانات الطالب
    return Ok(new
    {
        student.Id,
        student.Name,
...
public async Task<IActionResult> TransferStudent([FromBody] TransferStudentByIdRequest request)
{
    // Find student by ID directly
    var student = await db.Students
        .Include(s => s.School)
        .Include(s => s.Section)
            .ThenInclude(sec => sec!.Grade)
    ...
    if (!student.IsActive)
        return BadRequest(new { message = "الطالب غير نشط ولا يمكن نقله" });
    ...
    // Check if a student with the same local number exists in the target school
    var existingInTarget = await db.Students
        .FirstOrDefaultAsync(s => s.SchoolId == request.NewSchoolId && 
                                  s.LocalStudentNumber == student.LocalStudentNumber);

    // If conflict, generate new local number
    if (existingInTarget != null)
    {
        var maxLocalNumber = await db.Students
            .Where(s => s.SchoolId == request.NewSchoolId)
            .MaxAsync(s => (int?)s.LocalStudentNumber) ?? 0;
        
        student.LocalStudentNumber = maxLocalNumber + 1;
    }
...
public async Task<IActionResult> TransferEmployee(TransferEmployeeLocalRequest request)
{
    ...
    var currentSchoolId = oldEmployeeSchool.SchoolId;
