...
public async Task<IActionResult> GetStudents()
{
    var students = await db.Students
        ...
        .Select(s => new
        {
            ...
            // عدد الكتب المستعارة النشطة
            ActiveLoansCount = db.BookLoans.Count(l => l.StudentId == s.Id && l.Status == LoanStatus.Active),
            // عدد الكتب المتأخرة
            OverdueLoansCount = db.BookLoans.Count(l => l.StudentId == s.Id && l.Status == LoanStatus.Overdue),
            // عدد الحجوزات المعلقة
            PendingReservationsCount = db.BookReservations.Count(r => r.StudentId == s.Id && r.Status == ReservationStatus.Pending),
            s.CreatedAt
        })
        .ToListAsync();
...
public async Task<IActionResult> GetStudent(int localStudentNumber)
{
    ...
    // جلب الكتب المستعارة النشطة للطالب
    var activeLoans = await db.BookLoans
        .Include(l => l.Book)
        .Where(l => l.StudentId == student.Id && l.Status == LoanStatus.Active)
        .Select(l => new
        {
            l.Id,
            l.LocalLoanNumber,
            ...
            IsOverdue = l.expiryDate < DateOnly.FromDateTime(DateTime.Today),
            DaysOverdue = l.expiryDate < DateOnly.FromDateTime(DateTime.Today) ? 
                (DateOnly.FromDateTime(DateTime.Today).DayNumber - l.expiryDate.DayNumber) : 0
        })
        .ToListAsync();

    // جلب تاريخ الإعارات السابقة
    var loanHistory = await db.BookLoans
        .Include(l => l.Book)
        .Where(l => l.StudentId == student.Id && l.Status == LoanStatus.Returned)
    ...
    // جلب الحجوزات النشطة
    var activeReservations = await db.BookReservations
        .Include(r => r.Book)
        .Where(r => r.StudentId == student.Id && 
        ...
        .Select(r => new
        {
            ...
            StatusName = r.Status.ToString(),
            StatusArabic = r.Status == ReservationStatus.Pending ? "قيد الانتظار" :
                          r.Status == ReservationStatus.Approved ? "تمت الموافقة" : "غير معروف"
        })
        .ToListAsync();
    ...
    return Ok(new
    {
        ...
        data = new
        {
            ...
            Statistics = new
            {
                ...
                TotalLoans = activeLoans.Count + loanHistory.Count
            },
    ...
    [FromQuery] int? localSectionNumber = null)
{
    ...
    if (localStudentNumber.HasValue)
        query = query.Where(s => s.LocalStudentNumber == localStudentNumber.Value);
        ...
        [FromQuery] int? days = 14)
    {
        ...
        var loan = new BookLoan
        {
            ...
            date = today,
            expiryDate = today.AddDays(days ?? 14),
        ...
        await notifier.SendAsync(student.Id, UserType.Student,
            ...
            $"تم تسجيل استعارتك لكتاب \"{book.Title}\". مدة الاستعارة {days ?? 14} يوماً تنتهي في {loan.expiryDate:yyyy-MM-dd}",
            "loan_created");
        ...
        return Created($"api/librarian/loans/{loan.LocalLoanNumber}", new
        {
            ...
            data = new
            {
                ...
                Loan = new
                {
                    ...
                    date = loan.expiryDate.ToString("yyyy-MM-dd"),
                    expiryDate = loan.expiryDate.ToString("yyyy-MM-dd"),
                    loan.Status,
                    loan.CreatedAt
                }
            }
        ...
        [FromQuery] int? days = 14)
    {
        ...
        var loan = new BookLoan
        {
            ...
            LocalLoanNumber = newLocalNumber,
            date = DateOnly.FromDateTime(DateTime.Today),
            expiryDate = DateOnly.FromDateTime(DateTime.Today.AddDays(days ?? 14)),
            Status = LoanStatus.Active,
            CreatedAt = DateTime.UtcNow
        };
        ...
        [FromQuery] DateOnly expiryDate)
    {
        ...
        var daysDiff = (expiryDate.DayNumber - today.DayNumber);
        await notifier.SendAsync(student.Id, UserType.Student,
            "تم تسجيل استعارة كتاب",
            $"تم تسجيل استعارتك لكتاب \"{book.Title}\". مدة الاستعارة {daysDiff} يوماً تنتهي في {expiryDate:yyyy-MM-dd}",
            "loan_created");
        ...
        [FromQuery] int localBookNumber)
    {
        ...
        if (!string.IsNullOrEmpty(student.GuardianPhone))
        {
            await notifier.SendToGuardianAsync(student,
                "تم إعادة كتاب ",
                $"تم {student.Name} إعادة كتاب \"{book.Title}\" إلى المكتبة",
                "library_return");
        }
        ...
        [FromQuery] bool? overdue = null)
    {
        ...
        if (overdue == true)
            query = query.Where(l => l.Status == LoanStatus.Active && l.expiryDate < today);
        ...
        var loans = await query
            ...
            .Select(l => new
            {
                ...
                IsOverdue = l.Status == LoanStatus.Active && l.expiryDate < today,
                DaysOverdue = l.Status == LoanStatus.Active && l.expiryDate < today ? 
                    (today.DayNumber - l.expiryDate.DayNumber) : 0,
                l.CreatedAt
            })
            .ToListAsync();
        ...
        var loans = await db.BookLoans
            ...
            .Select(l => new
            {
                ...
                StatusArabic = l.Status == LoanStatus.Active ? "نشط" :
                              l.Status == LoanStatus.Returned ? "مُعاد" :
                              l.Status == LoanStatus.Overdue ? "متأخر" : "غير معروف",
                IsOverdue = l.Status == LoanStatus.Active && l.expiryDate < DateOnly.FromDateTime(DateTime.Today),
                DaysOverdue = l.Status == LoanStatus.Active && l.expiryDate < DateOnly.FromDateTime(DateTime.Today) ? 
                    (DateOnly.FromDateTime(DateTime.Today).DayNumber - l.expiryDate.DayNumber) : 0
            })
            .ToListAsync();
    ...
    public async Task<IActionResult> NotifyOverdueLoans()
    {
        ...
        foreach (var loan in overdueLoans)
        {
            ...
            {
                var daysOverdue = today.DayNumber - loan.expiryDate.DayNumber;
